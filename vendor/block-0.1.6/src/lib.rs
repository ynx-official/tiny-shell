...
// Opaque Objective-C runtime class. An empty enum is uninhabited and cannot be
// used as an extern static on future Rust versions; a zero-sized opaque struct
// preserves the address-only FFI representation without exposing internals.
#[repr(C)]
struct Class {
    _private: [u8; 0],
}
